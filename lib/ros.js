...
(function () {
  ...
  window.ros = {
    ...
    hincrby: function (key, field, count) {
      ...
      if (false !== value && false === vcount) return this._error("Error type of value");

      if (false === value) this.hset(key, field, count);
    ...
    lrange: function (key, start, end) {
      ...
      return end == -1 ? data[key].slice(start) : data[key].slice(start, end + 1);
    },
    ...
    zadd: function (key, score, value) {
      ...
      var return_value = 0;
      var inserted = false;
      if (!data[key].hasOwnProperty(value)) return_value = 1;
      data[key][value] = score;
      var len = sorted_list[key].length;
      for (var i = 0; i < len; i++) {
        if (data[key][sorted_list[key][i]] < score) {
          sorted_list[key].splice(index + 1, 0, value);
          inserted = true;
          break;
        }
      }
    ...
    zcount: function (key, start, end) {
      ...
      return end == -1 ? sorted_list[key].slice(start).length : sorted_list[key].slice(start, end + 1).length;
    },
    ...
    zrange: function (key, start, end, withscores) {
      ...
      var members = end == -1 ? sorted_list[key].slice(start) : sorted_list[key].slice(start, end + 1);
