describe('Set', function () {
    ...
    describe('sadd()', function () {
        ...
        it('Add value to illegal structure.', function () {
            ros.set('set.3', '3');
            ros.sadd('set.3', '4');
            assert('3' === ros.get('set.3'));
            assert(false === ros.smembers('set.3'))
        });

        it('Add object to set.', function () {
            var obj = {test: 'test'};
            ros.sadd('set.2', obj);
            assert(true === ros.sismember('set.2', obj));
            assert(['c', 'd', obj].toString() === ros.smembers('set.2').toString());
        });
    });

    describe('srem()', function () {
        it('Remove a member of set.', function () {
            assert(true === ros.srem('set.2', 'c'));
            assert(['d'].toString() === ros.smembers('set.2').toString())
        });

        it('Remove non-exist member of set.', function () {
            assert(false === ros.srem('set.2', 'b'));
        });
    ...
    describe('spop()', function () {
        it('Pop a value of set.', function () {
            assert('d' === ros.spop('set.2'));
            assert(['c'].toString() === ros.smembers('set.2').toString());
            assert('c' === ros.spop('set.2'));
            assert(null === ros.spop('set.2'))
        });

        it('Pop a value of non-exist set.', function () {
            assert(null === ros.spop('set.spop'));
        });
    ...
    describe('sinterstore()', function () {
        it('Store intersect of two set', function () {
            var ok = ros.sinterstore('set.3', 'set.1', 'set.2');
            assert(1 === ok);
            assert(['c'].toString() === ros.smembers('set.3').toString());
        });
