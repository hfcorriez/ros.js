  ...
  return {
    ...
    zadd: function (key, score, value) {
      ...
      if (data[key].hasOwnProperty(value)) {
        ...
        sorted_list[key].splice(ranks[key][value], 1);
        // Delete rank
        delete ranks[key][value];
      }

      // Set value
      data[key][value] = score;
      var len = sorted_list[key].length;
      for (var i = 0; i < len; i++) {
        if (data[key][sorted_list[key][i]] > score) {
          sorted_list[key].splice(i, 0, value);
          ranks[key][value] = i;
          inserted = true;
          break;
        }
    ...
    zremrangebyrank: function (key, start, stop) {
      ...
      var remove_members = sorted_list[key].splice(start, stop === -1 ? sorted_list[key].length : stop + 1);
      for (var i = 0, len = remove_members.length; i < len; i++) {
        delete data[key][remove_members[i]];
        delete ranks[key][remove_members[i]];
      }
    ...
    zremrangebyscore: function (key, min, max) {
      ...
      var num = 0;
      for (var i = 0, len = sorted_list[key].length; i < len; i++) {
        var value = data[key][sorted_list[key][i - num]];
        if (value >= min && value <= max) {
          sorted_list[key].splice(i - num, 1);
          delete data[key][value];
          delete ranks[key][value];
          num++;
        }
    ...
    zcount: function (key, start, end) {
      ...
      return sorted_list[key].slice(start, end === -1 ? sorted_list[key].length : end + 1).length;
    },
    ...
    zrange: function (key, start, end, withscores) {
      ...
      var members = sorted_list[key].slice(start, end === -1 ? sorted_list[key].length : end + 1);
      if (!withscores) return members;
    ...
    zrevrange: function (key, start, end, withscores) {
      ...
      var reverse_sorted_list = sorted_list[key].reverse();
      var members = reverse_sorted_list.slice(start, end === -1 ? sorted_list[key].length : end + 1);
      if (!withscores) return members;
    ...
    zrangebyscore: function (key, start, end, withscores) {
      ...
      var members = [];
      for (var i = 0, len = sorted_list[key].length; i < len; i++) {
        if (data[key][sorted_list[key][i]] >= start && data[key][sorted_list[key][i]] <= end) members.push(sorted_list[key][i]);
      }
      if (!withscores) return members;
    ...
    zrevrangebyscore: function (key, start, end, withscores) {
      ...
      var members = [];
      for (var len = sorted_list[key].length, i = len; i > 0; i--) {
        if (data[key][sorted_list[key][i]] >= start && data[key][sorted_list[key][i]] <= end) members.push(sorted_list[key][i]);
      }
      if (!withscores) return members;
    ...
    zrevrank: function (key, value) {
      ...
      return sorted_list[key].length - rank - 1;
    },
